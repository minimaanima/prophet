...
import { formatValidationErrors, ScanSchema, type Scan } from '@/lib/scan-schema';

const fallbackPositions = [
  { ticker: 'SKHY', name: 'SK hynix ADR', price: 171.23, day: 2.4, signal: 'ADD', score: 84, delta: 6 },
  { ticker: 'SGM', name: 'Revolut instrument', price: 45.18, day: 0.8, signal: 'ADD', score: 79, delta: 2 },
  { ticker: 'NVDA', name: 'NVIDIA', price: 218.42, day: -1.1, signal: 'HOLD', score: 75, delta: -3 },
  { ticker: 'MSFT', name: 'Microsoft', price: 513.08, day: -0.4, signal: 'HOLD', score: 73, delta: 0 },
  { ticker: 'TMUS', name: 'T-Mobile US', price: 181.34, day: 0.3, signal: 'HOLD', score: 70, delta: 1 },
  { ticker: 'AAPL', name: 'Apple', price: 320.16, day: 0.6, signal: 'WATCH', score: 62, delta: -5 },
];
...
export function ProphetDashboard() {
  ...
  const [importState, setImportState] = useState<{ kind: 'idle' | 'saving' | 'error' | 'success'; message?: string; errors?: Array<{ path: string; message: string }> }>({ kind: 'idle' });
  ...
  const averageScore = positions.reduce((sum, item) => sum + item.score, 0) / positions.length;
  const changedTheses = scan?.portfolio.filter((item) => item.thesis.changed_since_previous_scan).length ?? 2;
  ...
  async function saveImport() {
    ...
    try {
      ...
      setImportState({ kind: 'error', errors: [{ path: 'root', message: 'Invalid JSON syntax' }] });
      return;
    }

    const validation = ScanSchema.safeParse(value);
    if (!validation.success) {
      setImportState({ kind: 'error', errors: formatValidationErrors(validation.error) });
      return;
    }

    const response = await fetch('/api/scans', { method: 'POST', headers: { 'content-type': 'application/json' }, body: importText });
    const result = await response.json() as { ok?: boolean; scan?: Scan; errors?: Array<{ path: string; message: string }> };
    if (!response.ok || !result.ok || !result.scan) {
      setImportState({ kind: 'error', errors: result.errors ?? [{ path: 'root', message: 'Import failed' }] });
      return;
    }
    setScan(result.scan);
    setImportState({ kind: 'success', message: `${result.scan.portfolio.length} positions imported` });
  }
      ...
      <header className="sticky top-0 z-20 border-b border-white/8 bg-background/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-5 px-4 sm:px-7">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button className="ml-1" size="lg" onClick={() => setImportOpen(true)}><Import data-icon="inline-start" /> Import scan</Button>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-[1500px] gap-6 px-4 py-6 sm:px-7 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <section id="overview" className="min-w-0">
          <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <p className="font-mono text-[11px] uppercase tracking-[.16em] text-primary">{scan ? `${scan.run_type.replaceAll('_', ' ')} · ${new Date(scan.generated_at).toLocaleString('en-GB', { timeZone: 'Europe/Sofia', dateStyle: 'medium', timeStyle: 'short' })}` : 'Market close · 31 Aug 2026'}</p>
              <h1 className="mt-2 text-3xl font-semibold tracking-[-.04em] sm:text-4xl">Portfolio intelligence</h1>
              <p className="mt-2 max-w-xl text-sm text-muted-foreground">Price data and AI thesis tracking across every position.</p>
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-white/8 bg-card px-3 py-2 text-xs text-muted-foreground">
              <BrainCircuit className="size-4 text-primary" /> Last scan processed 12 min ago
            </div>
          </div>

          <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Metric label="Portfolio score" value={averageScore.toFixed(1)} detail="latest scan" positive />
            <Metric label="Tracked positions" value={String(positions.length)} detail={`${positions.filter((item) => item.signal === 'ADD').length} ADD · ${positions.filter((item) => item.signal === 'HOLD').length} HOLD`} />
            <Metric label="Thesis changes" value={String(changedTheses)} detail="since previous" positive />
            <Metric label="Next scan" value="09:00" detail="Europe / Sofia" />
          </div>

          <Card id="portfolio" className="border border-white/8 bg-card/80 ring-0">
            <CardHeader className="border-b border-white/8 pb-4">
              <CardTitle>Portfolio snapshot</CardTitle>
              <CardDescription>Latest price and assessment from the selected scan.</CardDescription>
            </CardHeader>
            <CardContent className="px-0">
              <Table>
                <TableHeader>
                  <TableRow className="border-white/8 hover:bg-transparent">
                    <TableHead className="pl-5 text-xs text-muted-foreground">Instrument</TableHead>
                    <TableHead className="text-right text-xs text-muted-foreground">Price</TableHead>
                    <TableHead className="text-right text-xs text-muted-foreground">Day</TableHead>
                    <TableHead className="text-center text-xs text-muted-foreground">Signal</TableHead>
                    <TableHead className="text-right text-xs text-muted-foreground">Score</TableHead>
                    <TableHead className="pr-5 text-right text-xs text-muted-foreground">Δ score</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {positions.map((position) => (
                    <TableRow key={position.ticker} className="cursor-pointer border-white/8 hover:bg-white/[.035]">
                      ...
                      <TableCell className="text-center"><Badge variant="outline" className={signalClass[position.signal]}>{position.signal}</Badge></TableCell>
                      <TableCell className="text-right font-mono font-semibold">{position.score}</TableCell>
                      <TableCell className={`pr-5 text-right font-mono ${position.delta > 0 ? 'text-emerald-300' : position.delta < 0 ? 'text-rose-300' : 'text-muted-foreground'}`}>{position.delta > 0 ? '+' : ''}{position.delta}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="max-h-[88vh] max-w-3xl overflow-hidden border border-white/10 bg-[#111923] p-0 shadow-2xl">
          <DialogHeader className="border-b border-white/8 px-5 py-4">
            <DialogTitle className="flex items-center gap-2"><FileJson className="size-4 text-primary" />Import scheduled scan</DialogTitle>
            <DialogDescription>Paste the JSON returned by ChatGPT or choose a local .json file. Nothing is sent outside this application.</DialogDescription>
          </DialogHeader>
          <div className="grid min-h-0 gap-4 px-5 sm:grid-cols-[minmax(0,1fr)_220px]">
            <Textarea
              aria-label="Scan JSON"
              value={importText}
              onChange={(event) => { setImportText(event.target.value); setImportState({ kind: 'idle' }); }}
            ...
            <div className="space-y-3">
              <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-white/15 px-3 py-4 text-xs text-muted-foreground transition hover:border-primary/50 hover:text-white">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    if (file) void file.text().then((text) => { setImportText(text); setImportState({ kind: 'idle' }); });
                  }}
                />
              </label>
              <Button variant="outline" className="w-full" onClick={loadExample}>Load valid example</Button>
              {importState.kind === 'success' && (
                <div className="rounded-lg border border-emerald-400/20 bg-emerald-400/8 p-3 text-xs text-emerald-200">
                  <div className="flex items-center gap-2 font-medium"><CheckCircle2 className="size-4" />Import complete</div>
                  <p className="mt-1 text-emerald-200/70">{importState.message}</p>
                </div>
              )}
              {importState.kind === 'error' && (
                <div className="max-h-72 overflow-auto rounded-lg border border-rose-400/20 bg-rose-400/8 p-3 text-xs text-rose-200">
                  <div className="mb-2 flex items-center gap-2 font-medium"><AlertCircle className="size-4" />Validation failed</div>
                  <ul className="space-y-2">
                    {importState.errors?.slice(0, 12).map((error, index) => <li key={`${error.path}-${index}`}><span className="font-mono text-rose-100">{error.path}</span><br /><span className="text-rose-200/70">{error.message}</span></li>)}
                  </ul>
                </div>
              )}
              <div className="rounded-lg border border-white/8 bg-white/[.025] p-3 text-[11px] leading-relaxed text-muted-foreground">
                Schema v1 requires strict signal, risk, thesis and run-type enums. Invalid responses are retained for troubleshooting.
              </div>
            </div>
          </div>
          <DialogFooter className="border-white/8 bg-white/[.025]">
            <Button variant="outline" onClick={() => setImportOpen(false)}>Close</Button>
            <Button disabled={!importText.trim() || importState.kind === 'saving'} onClick={saveImport}>{importState.kind === 'saving' ? 'Saving…' : 'Validate & save'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </main>
    ...
    <Card className="gap-2 border border-white/8 bg-card/80 py-4 ring-0">
      <CardContent>
        ...
        <div className="mt-2 flex items-end justify-between gap-3">
          ...
          <span className={`pb-1 text-[11px] ${positive ? 'text-emerald-300' : 'text-muted-foreground'}`}>{detail}</span>
        </div>
      </CardContent>
    </Card>
  );
...
const exampleScan = {
  ...
  market: 'US',
  market_summary: { sentiment: 'bullish', summary: 'Semiconductors led gains while defensive sectors lagged.' },
  portfolio: fallbackPositions.map((item) => ({
    ...
    currency: 'USD',
    price: { value: item.price, currency: 'USD', timestamp: '2026-08-31T20:00:00Z', previous_close: null, change: null, change_pct: item.day, source: 'scan' },
    assessment: { signal: item.signal, score: item.score, confidence: 0.8, risk: item.ticker === 'SKHY' || item.ticker === 'NVDA' ? 'high' : 'medium' },
    thesis: { status: item.delta > 0 ? 'improving' : item.delta < 0 ? 'deteriorating' : 'unchanged', summary: 'Latest evidence remains consistent with the monitored investment thesis.', changed_since_previous_scan: item.delta !== 0 },
    catalysts: [], risks: [], events: [], delta: { score_change: item.delta, signal_changed: false, thesis_changed: item.delta !== 0 }, summary: '',
  })),
  opportunities: [],
