...
export function WorkspacePageShell({
  ...
  useEffect(() => {
    void fetch('/api/quotes')
      .then(async (response) => {
        if (!response.ok) throw new Error('Market data status unavailable');
        return (await response.json()) as { configured?: boolean };
      })
      .then((data) => setMarketDataConfigured(Boolean(data.configured)))
      .catch(() => setMarketDataConfigured(false));
  }, []);
