...
export function PortfolioView() {
  ...
  return (
    <>
      <div className="mb-6">
        <p className="font-mono text-[11px] uppercase tracking-[.16em] text-primary">Positions</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-[-.04em] sm:text-4xl">Portfolio</h1>
        <p className="mt-2 text-sm text-muted-foreground">The latest saved assessment for every instrument in the imported scan.</p>
      </div>
      <Card className="border border-white/8 bg-card/80 ring-0">
        <CardHeader className="border-b border-white/8 pb-4">
          <CardTitle>Current positions</CardTitle>
          <CardDescription>{scan ? `${scan.portfolio.length} instruments · ${scan.run_type.replaceAll('_', ' ')}` : 'Waiting for an imported scan'}</CardDescription>
        </CardHeader>
        <CardContent className="px-0">
          {!loaded ? <Status message="Loading portfolio…" /> : !scan ? <Status message="No valid scan has been imported yet." /> : (
            <Table>
              <TableHeader><TableRow className="border-white/8 hover:bg-transparent"><TableHead className="pl-5">Instrument</TableHead><TableHead className="text-right">Price</TableHead><TableHead className="text-right">Day</TableHead><TableHead className="text-center">Signal</TableHead><TableHead className="text-right">Score</TableHead><TableHead className="pr-5 text-right">Risk</TableHead></TableRow></TableHeader>
              <TableBody>{scan.portfolio.map((item) => (
                <TableRow key={item.ticker} className="border-white/8 hover:bg-white/[.035]">
                  ...
                  <TableCell className="pr-5 text-right capitalize text-muted-foreground">{item.assessment.risk.replace('_', ' ')}</TableCell>
                </TableRow>
              ))}</TableBody>
            </Table>
          )}
...
function Status({ message }: { message: string }) {
  return <div className="grid min-h-64 place-items-center text-sm text-muted-foreground"><div className="text-center"><Database className="mx-auto mb-3 size-6 text-primary" />{message}</div></div>;
}
