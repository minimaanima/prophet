...
export async function GET() {
  ...
  const latest = await db.prepare(
    `SELECT id, raw_json, generated_at, run_type
     FROM scan_runs WHERE status = 'processed'
     ORDER BY generated_at DESC LIMIT 1`,
  ).first<{ id: string; raw_json: string; generated_at: string; run_type: string }>();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    ).bind(id, now, raw, JSON.stringify([{ path: 'root', message: 'Invalid JSON syntax' }]), now).run();
    return Response.json({ ok: false, id, errors: [{ path: 'root', message: 'Invalid JSON syntax' }] }, { status: 400 });
  }
  ...
  if (!result.success) {
    ...
    const id = typeof candidate.scan_run_id === 'string' ? candidate.scan_run_id : `invalid_${crypto.randomUUID()}`;
    await db.prepare(
      `INSERT INTO scan_runs
       (id, run_type, schema_version, generated_at, market, status, raw_json, error_json, created_at)
       VALUES (?, ?, ?, ?, ?, 'invalid', ?, ?, ?)
      ...
      typeof candidate.schema_version === 'string' ? candidate.schema_version : 'unknown',
      typeof candidate.generated_at === 'string' ? candidate.generated_at : now,
  ...
  const statements = [
    ...
    db.prepare('DELETE FROM events WHERE snapshot_id IN (SELECT id FROM snapshots WHERE scan_run_id = ?)').bind(scan.scan_run_id),
    db.prepare('DELETE FROM snapshots WHERE scan_run_id = ?').bind(scan.scan_run_id),
  ];
  ...
  for (const item of scan.portfolio) {
    ...
    statements.push(
      ...
      ).bind(item.ticker, item.name, item.exchange ?? null, item.currency, item.instrument_type, now),
      db.prepare(
  ...
  return Response.json({ ok: true, scan, imported: scan.portfolio.length }, { status: 201 });
}
